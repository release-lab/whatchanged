...
{{- define "body" -}}
{{range . -}}
- {{if .Field.Header.Scope }}**{{ .Field.Header.Scope | unescape }}**: {{ end }}{{ .Field.Header.Subject | unescape }}({{ .HashURL }}) (@{{ .Author.Name | unescape }}){{if .Closes }}, Closes: {{ range $index, $element := .Closes}}{{if $index}},{{end}}{{$element}}{{- end -}} {{- end }}
{{ end }}
{{- end -}}
